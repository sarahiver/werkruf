...
export function buildAlerts(r) {
  ...
  if (r.reviewCount < 20)
    list.push({ t: r.reviewCount < 5 ? 'err' : 'warn',
  ...
  if (r.rating > 0 && r.rating < 4.0)
    ...
    list.push({ t: 'warn',
      ...
      desc: 'Ab 4.5 Sternen steigt die Klickrate messbar.' });
  return list;
}
...
export function usePlacesAnalysis() {
  // phase: 'idle' | 'scanning' | 'result' | 'sent'
  ...
  const runAnalysis = useCallback(async (placeOption) => {
    ...
    const fetchPromise = fetchPlaceDetails(placeOption.value.place_id).catch(() => null);
