...
/* ─────────────────────────────────────────────
   ...
   Die Betriebs-Verknüpfung über Google Places. Lag früher auf der
   Dashboard-Startseite und hat sie überladen — dort geht es jetzt um
   Lage und nächste Handlung, nicht um Einrichtung.

   Der Inhalt ist unverändert: Betrieb suchen, verknüpfen, zurücksetzen,
   manuell eintragen, und der Zustand "noch kein Google-Eintrag".

   Verhältnis zur OAuth-Verbindung: Das hier setzt
   user_profiles.google_place_id und speist den Sichtbarkeits-Score.
   Die OAuth-Verbindung ist etwas anderes — sie erlaubt Schreibzugriff.
   Beides kann unabhängig voneinander bestehen, deshalb bleiben beide
   Karten auf der Profilseite nebeneinander.
───────────────────────────────────────────── */
...
export default function BusinessLinkCard() {
  const { profile, user, refreshProfile } = useAuthContext();

  const { brand, places, key: industryKey } = useIndustry();
  ...
  // Manuelles Eintragen, falls der Betrieb bei Google nicht auffindbar ist
