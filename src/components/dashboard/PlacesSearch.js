      ...
      .then(() => {
        ...
        const ac = new window.google.maps.places.Autocomplete(
          inputRef.current,
          {
            ...
            language:             'de',
            // Field masking — CamelCase for new API, snake_case for legacy
            fields: [
              // New API (Places v1)
              'id', 'displayName', 'formattedAddress',
              'location', 'addressComponents',
              'rating', 'userRatingCount', 'websiteURI',
              // Legacy fallback names (same request, Google picks what's available)
        ...
        ac.addListener('place_changed', () => {
          ...
          if (!place || (!place.id && !place.place_id)) {
            // User pressed Enter without selecting — treat as no result
          ...
          // Normalise — support both v1 (camelCase) and legacy (snake_case)
          const normalised = {
            ...
            reviewCount:
              place.userRatingCount     || place.user_ratings_total || 0,
            hasWebsite:
              !!(place.websiteURI || place.website),
            website:
              place.websiteURI || place.website || null,
            addressComponents:
